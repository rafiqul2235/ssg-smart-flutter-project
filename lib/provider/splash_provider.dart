...
import 'package:ssg_smart2/data/model/response/config_model.dart';
...
class SplashProvider extends ChangeNotifier {
  ...
  void getCurrencyData(String currencyCode) {
    _configModel?.currencyList?.forEach((currency) {
      if(currencyCode == currency.code) {
        ...
        _currencyIndex = _configModel?.currencyList!.indexOf(currency)??0;
        return;
      }
    });
  }
  ...
  bool showIntro() {
    return splashRepo.showIntro()??false;
  }
  ...
  Future<bool> testApi(BuildContext context) async {
    _hasConnection = true;

    print('Splash Provider Test ');

    ApiResponse apiResponse = await splashRepo.getTestHi();

    print('Splash Provider Test 74 ${apiResponse.response.toString()}');

    bool isSuccess;
    if (apiResponse.response != null && apiResponse.response?.statusCode == 200) {

      print('Splash Provider Test 79 ${apiResponse.response?.data}');
